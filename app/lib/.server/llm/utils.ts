...
export function simplifyWhiteLabelActions(input: string): string {
  // Using regex to match whiteLabelAction tags that have type="file"
  const regex = /(<whiteLabelAction[^>]*type="file"[^>]*>)([\s\S]*?)(<\/whiteLabelAction>)/g;
...
export function createFilesContext(files: FileMap, useRelativePath?: boolean) {
  ...
  const fileContexts = filePaths
    ...
    .map((path) => {
      ...
      return `<whiteLabelAction type="file" filePath="${filePath}">${codeWithLinesNumbers}</whiteLabelAction>`;
    });

  return `<whiteLabelArtifact id="code-content" title="Code Content" >\n${fileContexts.join('\n')}\n</whiteLabelArtifact>`;
}
