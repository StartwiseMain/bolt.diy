...
echo "Starting White Label update process..."
...
UPDATE_REPO="${WHITE_LABEL_UPDATE_REPO:-white-label/white-label}"
LATEST_RELEASE_URL=$(curl -s "https://api.github.com/repos/${UPDATE_REPO}/releases/latest" | grep "browser_download_url.*zip" | cut -d : -f 2,3 | tr -d \")
