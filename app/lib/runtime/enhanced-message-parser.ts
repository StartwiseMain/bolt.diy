...
export class EnhancedStreamingMessageParser extends StreamingMessageParser {
  ...
  private _hasDetectedArtifacts(input: string): boolean {
    return input.includes('<whiteLabelArtifact') || input.includes('</whiteLabelArtifact>');
  }
  ...
  private _wrapInArtifact(artifactId: string, filePath: string, content: string): string {
    ...
    return `<whiteLabelArtifact id="${artifactId}" title="${title}" type="bundled">
<whiteLabelAction type="file" filePath="${filePath}">
${content}
</whiteLabelAction>
</whiteLabelArtifact>`;
  }
  ...
  private _wrapInShellAction(content: string, messageId: string): string {
    ...
    return `<whiteLabelArtifact id="${artifactId}" title="Shell Command" type="shell">
