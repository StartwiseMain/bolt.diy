...
class DebugLogger {
  ...
  private async _collectAppInfo(): Promise<AppInfo> {
    ...
    try {
      if (typeof window !== 'undefined') {
        ...
        const workbenchStore = (window as any).__white_label_workbench_store;
  ...
  private _getCurrentModel(): string {
    try {
      ...
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('white_label_current_model');
  ...
  private _getCurrentProvider(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('white_label_current_provider');
  ...
  private _getProjectType(): string {
    try {
      if (typeof window !== 'undefined') {
        const stored = localStorage.getItem('white_label_project_type');
  ...
  private _getGitInfoFallback(): AppInfo['gitInfo'] {
    try {
      ...
      const stored = localStorage.getItem('white_label_git_info');
...
export async function downloadDebugLog(filename?: string): Promise<void> {
  try {
    ...
    link.href = url;
    link.download = filename || `white-label-debug-${new Date().toISOString().split('T')[0]}.txt`;
