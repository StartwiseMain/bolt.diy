...
export class WhiteLabelShell {
  ...
  async init(webcontainer: WebContainer, terminal: ITerminal) {
    ...
    const { process, commandStream, expoUrlStream } = await this.newWhiteLabelShellProcess(webcontainer, terminal);
  ...
  async newWhiteLabelShellProcess(webcontainer: WebContainer, terminal: ITerminal) {
    const args: string[] = [];
    const process = await webcontainer.spawn('/bin/jsh', ['--osc', ...args], {
      terminal: {
        cols: terminal.cols ?? 80,
...
export function newWhiteLabelShellProcess() {
  return new WhiteLabelShell();
}
