...
export class ImportExportService {
    ...
    try {
      ...
      return {
        ...
        core: {
          ...
          white_label_user_profile: this._safeGetItem('white_label_user_profile'),
          white_label_settings: this._safeGetItem('white_label_settings'),
          white_label_profile: this._safeGetItem('white_label_profile'),
        ...
        features: {
          // Feature flags
          viewed_features: this._safeGetItem('white_label_viewed_features'),
          developer_mode: this._safeGetItem('white_label_developer_mode'),
        ...
        ui: {
          // Tab configuration
          white_label_tab_configuration: this._safeGetItem('white_label_tab_configuration'),
        ...
        debug: {
          ...
          acknowledged_debug_issues: this._safeGetItem('white_label_acknowledged_debug_issues'),
          acknowledged_connection_issue: this._safeGetItem('white_label_acknowledged_connection_issue'),

          // Error logs
          error_logs: this._safeGetItem('error_logs'),
          white_label_read_logs: this._safeGetItem('white_label_read_logs'),
        ...
        updates: {
          ...
          last_acknowledged_update: this._safeGetItem('white_label_last_acknowledged_version'),
        },
  ...
  static async deleteAllChats(db: IDBDatabase): Promise<void> {
    ...
    localStorage.removeItem('white_label_chat_history');
  ...
  private static async _importComprehensiveFormat(data: any): Promise<void> {
    ...
    if (data.ui) {
      ...
      if (data.ui.white_label_tab_configuration) {
        try {
          this._safeSetItem('white_label_tab_configuration', data.ui.white_label_tab_configuration);
        } catch (err) {
    ...
    if (data.debug) {
      ...
      const debugLocalStorageKeys = [
        'white_label_acknowledged_debug_issues',
        'white_label_acknowledged_connection_issue',
        'error_logs',
        'white_label_read_logs',
      ];
    ...
    if (data.updates) {
      ...
      if (data.updates.last_acknowledged_update) {
        try {
          this._safeSetItem('white_label_last_acknowledged_version', data.updates.last_acknowledged_update);
        } catch (err) {
