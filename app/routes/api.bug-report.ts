...
const bugReportSchema = z.object({
  ...
  environmentInfo: z
    .object({
      ...
      whiteLabelVersion: z.string().optional(),
...
function formatIssueBody(data: z.infer<typeof bugReportSchema>): string {
  ...
  if (data.includeEnvironmentInfo && data.environmentInfo) {
    ...
    if (data.environmentInfo.whiteLabelVersion) {
      body += `- white-label: ${data.environmentInfo.whiteLabelVersion}\n`;
    }
  ...
  body += '---\n*Submitted via white-label bug report feature*';
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  try {
    ...
    const targetRepo =
      (context?.cloudflare?.env as any)?.BUG_REPORT_REPO || process.env.BUG_REPORT_REPO || 'white-label/white-label';
    ...
    const octokit = new Octokit({
      auth: githubToken,
      userAgent: 'white-label-bug-reporter',
    });
